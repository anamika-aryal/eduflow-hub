...
function HodGlobalSearch() {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        const res = await fetch(`${API_URL}/api/hod/search?q=${encodeURIComponent(trimmed)}`, {
          headers: { ...authHeader() },
        });
        if (!res.ok) return;
  ...
  function goToResult(result: SearchResult) {
    if (result.type === "student") {
      navigate({
        to: "/hod/students",
        search: { q: result.name, sem: result.sem ?? undefined, section: result.section ?? undefined },
      });
    } else if (result.type === "course") {
      navigate({ to: "/hod/courses", search: { q: result.name } });
    } else {
      navigate({ to: "/hod/teachers" });
    }
    setOpen(false);
    setQuery("");
    setResults([]);
  ...
  const grouped = (["student", "teacher", "course"] as const)
