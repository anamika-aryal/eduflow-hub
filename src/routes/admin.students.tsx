...
function StudentsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setLoadError(null);
      try {
        const res = await fetch(`${API_URL}/api/admin/students`, { headers: { ...authHeader() } });
  ...
  // Departments are derived from real student data instead of a fixed list,
  // so the drilldown always matches whatever departments actually exist.
  const departmentList = useMemo(() => {
    const set = new Set<string>();
    for (const s of students) if (s.department) set.add(s.department);
    return Array.from(set).sort();
  }, [students]);

  const deptCounts = useMemo(
    () => Object.fromEntries(departmentList.map((d) => [d, students.filter((s) => s.department === d).length])),
    [departmentList, students],
  );

  const semesterCounts = useMemo(() => {
    if (!dept) return {};
    const counts: Record<number, number> = {};
    for (let s = 1; s <= 8; s++) counts[s] = students.filter((st) => st.department === dept && st.semester === s).length;
    return counts;
  }, [dept, students]);
          ...
          {viewTarget && (
            <>
              ...
              <div className="grid gap-3 text-sm">
                ...
                <DetailRow icon={Users} label="Guardian" value={`${viewTarget.guardian_name ?? "—"} · ${viewTarget.guardian_phone ?? "—"}`} />
                <div className="rounded-xl border border-border/60 bg-background/50 px-3 py-2">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {viewTarget.courses_enrolled.length === 0 && (
                      <span className="text-xs text-muted-foreground">None yet</span>
                    )}
                    {viewTarget.courses_enrolled.map((c) => <Badge key={c} variant="secondary" className="rounded-lg">{c}</Badge>)}
                  </div>
                </div>
              </div>
              <DialogFooter>
